...
public class SectorRestStore implements SectorStore {
    @Override
    public void getSector(final GetSectorStoreCallBack callback, final Criteria filter) {
        ...
        Call<List<RestSector>> getSector = service.GetSector(3,new DateTime("2017-12-29"));
        getSector.enqueue(new Callback<List<RestSector>>() {
            @Override
            public void onResponse(Call<List<RestSector>> call, Response<List<RestSector>> response) {
                switch (response.code()) {
                    case 200:
                        List<RestSector> ListSectorResponse = response.body();
                        callback.onSuccess(filter.match(ListSectorResponse));
                        break;
                    /*case 401:
            ...
            @Override
            public void onFailure(Call<List<RestSector>> call, Throwable t) {
