...
public class LocalidadRestStore implements LocalidadStore {
    @Override
    public void getLocalidad(final GetLocalidadStoreCallBack callback, final Criteria filter) {
        ...
        Call<List<RestLocalidad>> getlocalidad = service.GetLocalidad(dt.toDateTime());
        getlocalidad.enqueue(new Callback<List<RestLocalidad>>() {
            @Override
            public void onResponse(Call<List<RestLocalidad>> call, Response<List<RestLocalidad>> response) {
                switch (response.code()) {
                    case 200:
                        List<RestLocalidad> ListLocalidadResponse = response.body();
                        callback.onSuccess(filter.match(ListLocalidadResponse));
                        break;
                    /*case 401:
            ...
            @Override
            public void onFailure(Call<List<RestLocalidad>> call, Throwable t) {
