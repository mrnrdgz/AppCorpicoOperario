...
public class MedidorRestStore implements MedidorStore {
    @Override
    public void getMedidor(final GetMedidorStoreCallBack callback, final Criteria filter) {
        ...
        Call<List<RestMedidor>> getmedidor = service.GetMedidor(3,new DateTime("2017-12-29"));
        getmedidor.enqueue(new Callback<List<RestMedidor>>() {
            @Override
            public void onResponse(Call<List<RestMedidor>> call, Response<List<RestMedidor>> response) {
                switch (response.code()) {
                    case 200:
                        List<RestMedidor> ListMedidorResponse = response.body();
                        callback.onSuccess(filter.match(ListMedidorResponse));
                        break;
                    /*case 401:
            ...
            @Override
            public void onFailure(Call<List<RestMedidor>> call, Throwable t) {
