...
public class TipoConexionRestStore implements TipoConexionStore {
    @Override
    public void getTipoConexion(final GetTipoConexionStoreCallBack callback, final Criteria filter) {
        ...
        Call<List<RestTipoConexion>> gettipoConexion = service.GetTipoConexion(3,dt.toDateTime());
        gettipoConexion.enqueue(new Callback<List<RestTipoConexion>>() {
            @Override
            public void onResponse(Call<List<RestTipoConexion>> call, Response<List<RestTipoConexion>> response) {
                switch (response.code()) {
                    case 200:
                        List<RestTipoConexion> ListTipoConexionResponse = response.body();
                        callback.onSuccess(filter.match(ListTipoConexionResponse));
                        break;
                    /*case 401:
            ...
            @Override
            public void onFailure(Call<List<RestTipoConexion>> call, Throwable t) {
